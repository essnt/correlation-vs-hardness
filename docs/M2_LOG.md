...
- partial η²=0.986–0.988（预注册线 0.14；cadical 子集口径——论文全可算
  臂口径为 0.985–0.988）；端点比 10^4.5（预注册线 10）
...
- Tobit 全量拟合收敛不良（loglik=nan）——如实报告，删失敏感性由双口径 JT+KM 承担
  【2026-09-19 订正注：本条为 2026-09-17 修正前实现的状态（墙钟截断行携带 NaN 进入
  似然，全量拟合不收敛）；2026-09-17 起按 AMEND-2 语义排除墙钟行后全量拟合收敛良好
  （loglik=−1721.7、σ=2.13、r 与 Δ 系数 p≈1e-194 与 1e-69），与双口径 JT+KM
  结论一致】
...
2. **提交攒批风险**：run_batch 每 25 行才 commit，中途 kill 最多丢 24 行
   在途计算（最坏 24×300s）。
3. **修复（commit a26fefd；作者内部历史提交，包内语义等价物见
   PRESPEC_AUDIT 的"内部提交哈希对照"表）**：
