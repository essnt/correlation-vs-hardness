...
t0 = d[0].get_text()
chk("作者块 Jin Song（且无旧序残留）", "Jin Song" in t0 and "Song Jin" not in t0)
...
chk("外层 PDF 原始字节无构建机身份/路径", _S.encode() not in pdfb and _HP.encode() not in pdfb)
# 常见错拼与重复词
...
chk("快照 PDF 原始字节无构建机身份/路径", _S.encode() not in _arx and _HP.encode() not in _arx)
