...
\renewcommand{\shortauthors}{Song}
...
%% The class's metadata block in \maketitle expands \metadata@authors empty
%% (probed: hyperref's pdfauthor key never reaches the Info dict under
%% jair.cls), so the /Author entry is pinned via the pdfTeX primitive, after
%% \maketitle, to match the printed byline exactly.
\AtEndDocument{\pdfinfo{/Author (Jin Song)}}
