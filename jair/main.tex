...
the 9 radii; $r{=}0.06$ anchors are the lattice-nearest S0 grid value 2.6 (the grid
brackets the true boundary below 2.5) --- conservative for the span) $\times$ 30 paired seeds $\times$ two
...
$\Delta \in \{+0.2, +0.4, +0.8\}$. At $r{=}0.06$ the dosing anchor is the
lattice-nearest S0 value 2.6 rather than a measured crossing (the grid
brackets the true boundary below 2.5), so the realized offsets there exceed
the nominal $\Delta$ --- conservative for the gradient.
...
Together with E1 (\S\ref{sec:e1}) --- width-controlled instances stay flat
...
arm is uniformly \textsc{unsat} at this density and refuted in
single-digit-to-low-teens conflicts, with per-$k$ medians (pooling both
...
($\Delta{=}{+}0.8$) against the preregistered threshold of $0.14$.}
\label{tab:e3afull}
\begin{tabular}{lcccc}
\toprule
