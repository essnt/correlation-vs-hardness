...
neighborhoods, constraint solvers should scale well. Observational evidence
...
flips the satisfiability status of 53 instance pairs; the ratio is a
composite spanning those status changes (within the 29 status-stable pairs
the elevation persists at $\times 11$ and $\times 1.8$ per offset arm,
$p\le 3.9\times10^{-3}$); post-hoc, exploratory
causal mediation analysis is consistent with ${\sim}94\%$ of the radius effect
flowing through community-structure
restructuring (ACME 3.06, bootstrap CI $[2.84, 3.37]$). Locality is not merely
correlated with tractability --- under our controls it behaves as the causal channel.
...
a bounded amount, on the order of $0.1$--$0.2$ decades.
...
AMEND-5, dated 2026-09-18, first appeared with the deposit's fourth
version --- we disclose this
freeze-to-publication gap rather than let ``preregistered'' imply an
independent registry. Throughout, \emph{unblinding} denotes the first
examination of main-scan outcome statistics; the S0 thresholds that dose the
scan are examined beforehand by design, and the audit and its one material
fix predate unblinding.
...
random-3-SAT threshold region~\cite{mezard2002} ($\alpha_c \to 4.267$ as $n\to\infty$),
...
budget and 9 the $300$\,s wall guard, which excludes at scale only in the
S0 screen (313 rows). Figure~\ref{fig:dose}
...
Each arm attempts 120 swap pairings (4 radii $\times$ 30 seeds; the
original-arm rows are shared with the main scan and not re-stored);
completions fall to 33 and 49 because the large-radius cells saturate the
budget in either arm (at $+0.2$ the original arm decides $9/30$ and $0/30$
at $r{=}0.3$ and $r{=}1.5$, and the swap arm $0/30$ at both).

Together with E1, whose width-controlled instances are flat and trivial at all
...
(suppression: direct effect $-0.66$ --- consistent with a secondary channel,
though marginal single-mediator fits can also produce this pattern under
misspecification, so no claim rests on it); mean degree $2.34\ [2.15, 2.60]$; clustering
$2.19\ [2.02, 2.44]$. All CIs exclude zero; the mediator set was adapted
after the outcomes were known (AMEND-5), so the block is read as
exploratory, supportive mechanism evidence rather than a confirmatory test,
and the confirmatory weight rests with E3b.
...
FDR over the primary family, and censoring-robust cross-checks. No a priori
power analysis was performed: 30 seeds per cell was set by the compute
budget at design time, and the negative-branch trigger ($\eta^2<0.02$ with
$p>0.05$) carries no formal power calculation --- the confirmatory branch is
insensitive to this (observed effects exceed the thresholds by orders of
magnitude), but the negative branch's interpretability would have rested on
it.
...
AI-assisted tools were used in an assistive capacity for literature search and
citation verification, implementing the generators, solvers, and statistical
pipelines, executing the experiment matrix, and manuscript preparation (full
...
\bibitem{mezard2002} M.~M\'ezard, G.~Parisi, R.~Zecchina. Analytic and
algorithmic solution of random satisfiability problems. \emph{Science}
